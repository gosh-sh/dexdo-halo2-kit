...
//!
//! # STATUS (feature/multithreading migration): DISABLED
//!
//! These fixtures pre-date the X/Y split in `DarkDexCircuit::new` (BC-011,
//! 13-public layout): they carry a single-block schema (`block_id_hex`,
//! `envelope_hash_hex`, `block_proof_siblings_hex`, ...) and lack the Y-side
//! fields the current circuit expects (`y_block_id`, `y_envelope_hash`,
//! `y_tracked_ext_out_messages_root`, `x_block_id_h07_sibling`, ...).
//!
//! Re-enabling this file requires regenerating fixtures from an acki-nacki
//! node running the updated exporter, then rewriting `parse_fixture`,
//! `compute_instances`, and every `DarkDexCircuit::new*` call site against the
//! new (X, Y) shape. Until that lands, the file is gated behind the
//! `legacy_dex_fixtures` cargo feature (not declared in Cargo.toml) so it
//! compiles out entirely and does not break `cargo check`.
//!
//! When re-enabled, will run via:
//! ```bash
//! cargo test --features legacy_dex_fixtures test_real_data_mock_prover -- --nocapture
//! cargo test --release --features legacy_dex_fixtures test_real_data_real_prover -- --nocapture
//! ```
#![cfg(feature = "legacy_dex_fixtures")]
...
/// Append the 3 trailing public inputs to the instance vector:
///   [4] ephemeral_pubkey
