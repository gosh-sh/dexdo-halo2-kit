...
//! beyond `L_MAX = 20` without changing the per-snark circuit. With
...
//!
//! Cost: 10 real KZG proofs at circuit K = 19 ≈ 10 × ~100 s ≈ ~17 min
//! prove + ~60 s keygen ≈ ~18 min total.
//!
//! `#[ignore]`; run with
...
/// L = 50 (50-hop chain): `N_BUNDLE = 10` snarks × `H = 5` active hops each.
/// Every snark fully active, end-to-end continuity. `#[ignore]` due to
/// multi-minute real KZG cost (10 proofs at ~100s each ≈ 17 min).
#[test]
#[ignore]
fn bundle_stress_l50_n10() {
    const SEED: u64 = 0x0050_BEEFu64; // distinct from 2d/2f seeds
    const N_BUNDLE: usize = 10;
    const L_HOPS: usize = N_BUNDLE * H_HOPS_PER_PROOF; // 50

    // -- 1. Synth chain (L = 50, n_bundle = 10) --------------------------
    ...
    for (s_idx, snark) in snarks.iter().enumerate() {
        for (h_idx, hop) in snark.hops.iter().enumerate() {
            assert!(
                hop.is_active,
                "snark {s_idx} hop {h_idx} expected active at L = 50"
            );
        }
    }
    ...
    // -- 2. Keygen (single VK/PK shared across all 10 snarks) ------------
    ...
    // -- 3. Prove + verify each of the 10 snarks -------------------------
    ...
    verify_bundle(&bundle)
        .expect("bundle_verifier rejected the L = 50 / n_bundle = 10 bundle");
